...
def roll_dice(num_rolls, dice=six_sided):
    ...
    if flag:
        ...
        return res
    """
    res = [dice() for i in range(num_rolls)]
    if 1 in res:
        return 1
    return sum(res)
    # END PROBLEM 1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 += take_turn(dice_num, score0, score1, dice=dice, goal=GOAL_SCORE)
            score0 += pigs_on_prime(score0, score1)
        else:
            dice_num = strategy1(score1, score0)
            score1 += take_turn(dice_num, score1, score0, dice=dice, goal=GOAL_SCORE)
            score1 += pigs_on_prime(score1, score0)
        leader, mesg = say(score0, score1, leader)
        if mesg is not None:
            print(mesg)
        who = next_player(who)
    # END PROBLEM 5
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if new_leader == None or new_leader == last_leader:
        mesg = None
    else:
        mesg = f'Player {new_leader} takes the lead by {abs(score0 - score1)}'
    return new_leader, mesg
    # END PROBLEM 6
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if pigs_on_prime(score + oink_points(score, opponent_score), opponent_score) > 0:
        return 0
