...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        ptr = self.place
        distance = 0
        while not ptr.is_hive:
            if ptr.bees != [] and self.lower_bound <= distance <= self.upper_bound:
                return random_bee(ptr.bees)
            distance += 1
            ptr = ptr.entrance
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def __init__(self, health=1):
        """Create HungryAnt with a HEALTH quantity."""
        super().__init__(health)
            ...
            if pos.ant is not None:
                ...
                if pos.ant.is_container and pos.ant.ant_contained is not None:
                    ...
                    if not pos.ant.ant_contained.is_doubled:
                        pos.ant.ant_contained.damage *= 2
